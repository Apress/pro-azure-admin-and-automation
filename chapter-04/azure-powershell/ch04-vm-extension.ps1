...
$settings = '{"fileUris": ["https://apressazurebook.blob.core.windows.net/scripts/install-nginx.sh"],"commandToExecute": "sh install-nginx.sh"}'
Set-AzVMExtension -ResourceGroupName $resourceGroup -Location $location -VMName $vmName -Name "CustomScript" -Publisher "Microsoft.Azure.Extensions" -Type "CustomScript" -TypeHandlerVersion "2.1" -SettingString $settings
